...
export function BestsellersSection() {
  ...
  useEffect(() => {
    api
      .get<ApiProduct[]>("/products/featured")
      .then((data) => {
        if (data && data.length > 0) {
          setProducts(data.map(mapApiProductToProduct));
        } else {
          setProducts(fallbackProducts.filter((p) => p.isBestseller));
        }
      })
      .catch(() => {
        setProducts(fallbackProducts.filter((p) => p.isBestseller));
  ...
  return (
    <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-6 bg-[#FCEEEF]">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-10 sm:mb-14">
          <div>
            <p className="text-xs tracking-[0.4em] text-[#D97D93] uppercase mb-4">Mais Pedidos</p>
